...
of regular values of \(f\). Then \(R^1 f_* \Q_X|_{B^\circ}\) is a weight one variation 
of Hodge structures.

As a Hodge module, \(\Q_X(0)\) is pure of weight 0. Over \(B^\circ\), its pushforward 
decomposes as 
\begin{equation}
    \Q_{B^\circ}(0) \oplus R^1 f_* \Q_X|_{B^\circ}[-1] \oplus \Q_{B^\circ}(-1)[-2].
\end{equation}
Now let \(\Delta = B \smallsetminus B^\circ\). Suppose further that the singular fibers 
of \(f\) are irreducible. Over \(p \in \Delta\), we get 
\begin{equation}
    Rf_* \Q_X|_p = H^0(X_p, \Q)[2] \oplus H^1(X_p, \Q)[1] \oplus H^2(X_p, \Q)[0].
\end{equation}
